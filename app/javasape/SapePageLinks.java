...
public class SapePageLinks {
    ...
    public SapePageLinks(SapeConnection sapeConnection, String sapeUser, String requestUri, Cookies cookies) { //Cookie[] cookies from me
        this(sapeConnection, sapeUser, requestUri, cookies, false);
    }
    
    @SuppressWarnings("unchecked")
    public SapePageLinks(SapeConnection sapeConnection, String sapeUser, String requestUri, Cookies cookies, boolean showCode) { //Cookie[] cookies
//        if (sapeUser.equals(getCookieValue(cookies, "sape_cookie")))
        if (sapeUser.equals(cookies.get( "sape_cookie")))
            showCode = true;
    ...
    public String render(int count) {
        ...
        if (count < 0) count = pageLinks.size();
        
        for (Iterator<String> i = pageLinks.iterator(); i.hasNext() && count > 0; count --) {
            if (s.length() > 0) s.append(linkDelimiter);
